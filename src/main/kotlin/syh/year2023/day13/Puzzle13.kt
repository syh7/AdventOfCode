...
fun main() {
    val patterns = readDoubleLineFile("year2023/day13/actual.txt")
        .map { readPattern(it) }
    val alternativePatterns = patterns.map { findAlternativePattern(it) }

    println()
    printSum(patterns, "part a")
    printSum(alternativePatterns, "part b")
}

private fun printSum(patterns: List<Pattern>, partStr: String) {
