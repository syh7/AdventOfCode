...
fun main() {
    val lines = readSingleLineFile("year2023/day14/actual.txt")
...
//    println("leftover steps: $leftoverSteps")
    ...
    for (i in 0 until leftoverSteps) {
        cycleGrid(grid)
    }
...
fun tryToMoveBoulderNorth(grid: Grid, startI: Int, j: Int) {
    if (startI == 0) {
        return
    }
...
fun tryToMoveBoulderSouth(grid: Grid, startI: Int, j: Int) {
    if (startI == grid.size - 1) {
        return
    }
    for (i in startI until grid.size - 1) {
//        println("checking if [$i][$j] can move north to [${i - 1}][$j]")
...
fun tryToMoveBoulderWest(grid: Grid, i: Int, startJ: Int) {
    if (startJ == 0) {
        return
    }
...
fun tryToMoveBoulderEast(grid: Grid, i: Int, startJ: Int) {
    if (startJ == grid.size - 1) {
        return
    }
    for (j in startJ until grid[i].size - 1) {
        if (grid[i][j + 1] == EMPTY) {
            grid[i][j + 1] = BOULDER
            grid[i][j] = EMPTY
        } else {
