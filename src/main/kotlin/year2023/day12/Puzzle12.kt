...
fun main() {
    val springArrangements = readSingleLineFile("year2023/day12/actual.txt")
        .map { line ->
    ...
    springArrangements.forEach {
        ...
        val sumA = smartishDifferentSpringOptions(
            it.springInfo,
            it.expectedSprings,
            0,
            0,
        ...
        val sumB = smartishDifferentSpringOptions(
        ...
        println("calculated sum for ${it.springInfo} is $sumA")
        println("calculated sum for expanded ${it.springInfo} is $sumB")
        println()
    ...
    println("total different arrangements for plain string: $totalA")
    println("total different arrangements for expanded string: $totalB")
}

fun bruteForceDifferentSpringOptions(springInfo: String, expectedSprings: MutableList<Int>): Int {
    return if (springInfo.contains(OPTIONAL)) {

        val springWithReplacedSpring = springInfo.replaceFirst(OPTIONAL, SPRING)
        val springWithReplacedEmpty = springInfo.replaceFirst(OPTIONAL, EMPTY)

        val differentSpringOptions = bruteForceDifferentSpringOptions(springWithReplacedSpring, expectedSprings)
        val differentEmptyOptions = bruteForceDifferentSpringOptions(springWithReplacedEmpty, expectedSprings)

        differentSpringOptions + differentEmptyOptions

    } else {
        val springs = springInfo.split(EMPTY).filter { it.isNotEmpty() }.map { it.length }
        if (springs == expectedSprings) 1 else 0
    }
}

fun smartishDifferentSpringOptions(
    springString: String,
