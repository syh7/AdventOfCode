...
fun pipeTypeFromString(str: String): PipeType {
    return PipeType.values().first { it.stringValue == str }
}
...
typealias Grid = MutableList<MutableList<Pipe>>
...
fun main() {
    val lines = readSingleLineFile("year2023/day10/actual.txt")
...
private fun findCircle(grid: Grid): List<Pipe> {
...
private fun findNeighbourForPipe(origin: Pipe, grid: Grid): List<Pipe> {
...
private fun findNeighboursForStart(startPipe: Pipe, grid: Grid): List<Pipe> {
