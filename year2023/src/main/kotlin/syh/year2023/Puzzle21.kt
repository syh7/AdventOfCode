...
//    graph.forEach { line -> println(line.joinToString("") { it.toPreviousAndDistanceStr() }) }
    ...
    private fun partA(graph: MutableList<MutableList<Node>>): Long {
        ...
        return evenNodes.size.toLong()
    }


    private fun performDijkstra(graph: MutableList<MutableList<Node>>, startNode: Node) {
    ...
    private fun readGraph(input: List<List<String>>): MutableList<MutableList<Node>> {
        val allNodes: MutableList<MutableList<Node>> = mutableListOf()
    ...
    private fun findNeighbours(source: Node, graph: MutableList<MutableList<Node>>): List<Node> {
    ...
    private fun partB(graph: MutableList<MutableList<Node>>): Long {
...
//    println("calculated total even corner tiles = $totalEvenCornerTiles")
    ...
    data class Node(
        ...
        override fun toString(): String {
            val previousStr = if (previous != null) "with previous [${previous!!.x}][${previous!!.y}]" else "without previous"
